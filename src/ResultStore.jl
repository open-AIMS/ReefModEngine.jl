...
mutable struct ResultStore
    ...
    end_year::Int
    year_range::Int
    iv_outplant_start_years::Vector
    iv_outplant_end_years::Vector
    iv_outplant_year_steps::Vector
    iv_outplants_per_year::Vector
    iv_enrich_start_years::Vector
    iv_enrich_end_years::Vector
    iv_enrich_year_steps::Vector
    iv_enrich_per_year::Vector
    n_reefs::Int
    reps::Int
end
...
function save_result_store(dir_name::String, result_store::ResultStore)::Nothing
    ...
    interventions_dict = Dict(
        :iv_outplant_start_years => result_store.iv_outplant_start_years,
        :iv_outplant_end_years => result_store.iv_outplant_end_years,
        :iv_outplant_year_steps => result_store.iv_outplant_year_steps,
        :iv_outplants_per_year => result_store.iv_outplants_per_year,
        :iv_enrich_start_years => result_store.iv_enrich_start_years,
        :iv_enrich_end_years => result_store.iv_enrich_end_years,
        :iv_enrich_year_steps => result_store.iv_enrich_year_steps,
        :iv_enrich_per_year => result_store.iv_enrich_per_year
    )
...
function n_corals_calculation(
    ...
    return round(
        Int,
        (
            2 * sum(count_per_year) *
            (
                (iv_area / 100) * sum(target_reef_area_km²) *
                (1 / m2_TO_km2)
            )
        ) / n_years
    )
...
function append_scenarios!(rs::ResultStore, reps::Int)::Nothing
    ...
    # Count per m2 per year
    enrich_count_per_year::Vector{Float64} = zeros(6)
    # Area percentage
    enrichment_area::Float64 = 0.0
    # Number of locations
    enrichment_locs::Float64 = 0.0

    # Starting years in which enrichment occured
    enrich_first_years::Vector{Union{Int64,Any}} = []
    # Final years in which enrichment occured
    enrich_last_years::Vector{Union{Int64,Any}} = []
    # Frequency with which enrichment occured
    enrich_years_step::Vector{Union{Int64,Any}} = []
    # Number of corals used in enrichment each year
    enrich_corals_per_year::Vector{Union{Int64,Any}} = []
    ...
    for iv_idx in 1:n_iv
        ...
        if type == "outplant"
            ...
            @getRME ivOutplantCountPerM2(
                name::Cstring,
                outplant_count_per_year::Ptr{Cdouble},
                length(outplant_count_per_year)::Cint
            )::Cint
            ...
            push!(
                outplant_corals_per_year,
                n_corals_calculation(
                    outplant_count_per_year, iv_outplant_area, target_reef_area_km², n_years
                )
            )
        elseif type == "enrich"
            iv_enrich_area::Float64 = @getRME ivEnrichAreaPct(name::Cstring)::Cdouble
            n_enrichment_iv += n_years
            @getRME ivEnrichCountPerM2(
                name::Cstring,
                enrich_count_per_year::Ptr{Cdouble},
                length(enrich_count_per_year)::Cint
            )::Cdouble
            enrichment_count +=
                n_years * 2 * sum(enrich_count_per_year) # Multiplied by 2 is this is the amount per season with 2 seasons per year
            enrichment_area += n_years * @getRME ivEnrichAreaPct(name::Cstring)::Cdouble
            enrichment_locs += n_years * n_locs[1]
            push!(enrich_first_years, first_year)
            push!(enrich_last_years, last_year)
            push!(enrich_years_step, year_step)
            push!(
                enrich_corals_per_year,
                n_corals_calculation(
                    enrich_count_per_year, iv_enrich_area, target_reef_area_km², n_years
                )
            )
        end
    end
    ...
    if size(rs.scenario) == (0, 0)
        ...
        rs.iv_outplant_start_years = [outplant_first_years]
        rs.iv_outplant_end_years = [outplant_last_years]
        rs.iv_outplant_year_steps = [outplant_years_step]
        rs.iv_outplants_per_year = [outplant_corals_per_year]
        rs.iv_enrich_start_years = [enrich_first_years]
        rs.iv_enrich_end_years = [enrich_last_years]
        rs.iv_enrich_year_steps = [enrich_years_step]
        rs.iv_enrich_per_year = [enrich_corals_per_year]
    else
        rs.scenario = vcat(rs.scenario, df_cf, df_iv)
        rs.iv_outplant_start_years = [rs.iv_outplant_start_years..., outplant_first_years]
        rs.iv_outplant_end_years = [rs.iv_outplant_start_years..., outplant_last_years]
        rs.iv_outplant_year_steps = [rs.iv_outplant_year_steps..., outplant_years_step]
        rs.iv_outplants_per_year = [rs.iv_outplants_per_year..., outplant_corals_per_year]
        rs.iv_enrich_start_years = [rs.iv_enrich_start_years..., enrich_first_years]
        rs.iv_enrich_end_years = [rs.iv_enrich_end_years..., enrich_last_years]
        rs.iv_enrich_year_steps = [rs.iv_enrich_year_steps..., enrich_years_step]
        rs.iv_enrich_per_year = [rs.iv_enrich_per_year..., enrich_corals_per_year]
    end
